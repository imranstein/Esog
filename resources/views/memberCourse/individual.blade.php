        ...
        <tr>
            ...
            <td class="td-actions text-right">
                <div class="flex items-center space-x-4 text-sm">
                    @if($course->is_approved != null && $course->started_at == null)
                    <a class="btn btn-success" href="{{ route('startCourse',$course->course_id) }}">Start</a>
                    {{-- <a class="btn btn-primary" href="{{ route('course.show',$course->course_id) }}">Edit</a> --}}
                    @elseif($course->is_approved != null && $course->started_at != null && $course->finished_at == null)
                    <a class="btn btn-info" href="{{ route('course.show',$course->course_id) }}">Show</a>
                    @elseif($course->is_approved != null && $course->started_at != null && $course->finished_at != null)
                    <p>Finished</p>
                    @endif
