    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('title_lower', function (Project $model) {
            ...
            ->addColumn('funded_by')
            ->addColumn('sites')
            ->addColumn('start_date_formatted', fn (Project $model) => Carbon::parse($model->start_date)->format('d/m/Y'))
            ->addColumn('end_date_formatted', fn (Project $model) => Carbon::parse($model->end_date)->format('d/m/Y'))
            // show only 100 characters of the description
            ->addColumn('description', fn (Project $model) => substr(e($model->description), 0, 100));
    }
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('project-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('project-delete'))
                ->hide(),
        ];
    }
}
